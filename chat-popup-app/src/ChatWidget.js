...
import ChartWidget from "./widgets/ChartWidget"; // Importing the new ChartWidget
...
export default function ChatWidget({ onClose }) {
    ...
    const handleSend = async (text) => {
        ...
        try {
            ...
            if (lowerText.includes("latest trend")) {
                response = {
                    type: "chart",
                    data: [12, 19, 3, 5, 2, 3] // Example data for the chart
                };
            } else if (lowerText.includes("top 10 ranks") || lowerText.includes("get top 10")) {
                response = {
                    type: "list",
                    data: [
        ...
        return (
            ...
            >
                <Message.CustomContent>
                    ...
                    {type === "chart" && <ChartWidget data={content} />} {/* Rendering the ChartWidget */}
                    {!["list", "csv", "text", "chart"].includes(type) && <TextWidget text={content} />}
                </Message.CustomContent>
            </Message>
        );
    };
        ...
        <div style={{
            ...
            <MainContainer>
                <ChatContainer>
                    ...
                    <MessageInput
                        placeholder="Ask me about rankings, reports, trends, or analysis..."
                        onSend={handleSend}
                        disabled={isTyping}
                    />
                </ChatContainer>
